; (function ($, window, document, undefined) {
	...
	Plugin.prototype.init = function () {
		...
		$elem
			...
			.on('foucsout', {'plugin': plugin}, plugin.startTimer);
	...
	Plugin.prototype.startTimer = function (event) {
		...
		plugin.timer = setInterval(plugin.gotoSlide, plugin.options.animationSpeed, {
			'data': {
				'plugin': plugin,
				'slide': 'right'
			}
	...
	Plugin.prototype.gotoSlide = function (event) {
		...
		$active.addClass(direction).on(transevent, {
			'next': $next,
			'dir': direction
		}, function (event) {

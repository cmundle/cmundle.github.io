;(function ( $, window, document, undefined ) {
	...
	var pluginName = 'ik_treemenu',
		defaults = {
			'instructions': 'Use up or down arrows to move through menu items, and Enter or Space bar to toggle sub menus open and closed.',
	 ...
	 * @param {number} [options.menuTitle] - Menu title appears above the tree.
	...
	Plugin.prototype.init = function () {
		...
		$elem // set all li elements as tree folders and items
			...
			.each(function(i, el) {
				...
				$me.attr({
					'id': id + '_menuitem_' + i,
					'role': 'treeitem', // assign tree item role
					'tabindex': -1, // remove from tab order
					'aria-level': $me.parents('ul').length, // add tree level
					'aria-setsize': $me.siblings().length + 1, // define number of tree items on the current level
					'aria-posinset': $me.parent().children().index($me) + 1 // define position of the current element on the current level
					});

				$($me.contents()[0]).wrap('<span></span>'); // wrap text element of each tree item with span element

				if ($me.children('ul').length) {  // if the current tree item has sub menu
			...
			$elem // make the first tree item focusable
				.find('li:first')
				.attr({
						'tabindex': 0
				});
	 ...
	 * Selects tree item.
	 *
	 * @param {object} $item - jQuery object containing tree item to select.
	...
	Plugin.prototype.selectItem = function($item, plugin) {
		...
		$elem.find('.focused') // remove highlight form previously selected tree item
			.removeClass('focused');

		$elem.find('li').attr({ // remove all tree items from tab order
			'tabindex': -1
		});

		$item.attr({ // select specified tree item
			'tabindex': 0, // add selected tree item to tab order
			'aria-selected': true
		});

		if ($item.children('ul').length) { // highlight selected tree item
	 ...
	 * Toggles sub menu.
	 *
	 * @param {object} $item - jQuery object containing tree item with sub menu.
	 */
	Plugin.prototype.toggleSubmenu = function($item) {

		if($item.children('ul').length) { // check if the tree item contains sub menu
	...
	Plugin.prototype.onMouseOver = function (event) {
		...
		plugin.element // remove highlight form previous tree item
			.find('.mouseover')
			.removeClass('mouseover');

		$me.children('span') // add highlight to currently selected tree item
			.addClass('mouseover');
